...
fn main() {
  unsafe {
    let mut term = DwarfTerm::new(TILE_GRID_WIDTH, TILE_GRID_HEIGHT, "Dwarf Term").expect("WHOOPS!");
    ...
    while running {
      ...
      term
        .get_foreground_mut((watcher_position.0 as usize, watcher_position.1 as usize))
        .map(|mut_ref| *mut_ref = rgb32!(255, 255, 225));
    
      term
        .get_id_mut((watcher_position.0 as usize, watcher_position.1 as usize))
        .map(|mut_ref| *mut_ref = '@' as u8);
